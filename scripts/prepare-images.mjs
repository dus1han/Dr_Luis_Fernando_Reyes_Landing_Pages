...
/*
 * A seventh card, supplied later and dropped one level up rather than into
 * "B A". Read from its own path instead of being moved, so the folder the
 * clinic hands over stays exactly as they left it — move it into "B A" and
 * it can join the loop above.
 *
 * `ba-7`, NOT a renumber of the six above. Reusing an existing name for
 * different bytes is how you get Next's image optimiser serving the old
 * picture from `.next/cache/images` behind correct markup — it caches by
 * request URL, and the URL wouldn't have changed. See the page reference.
 */
await emit(
  "ba-7.jpg",
  sharp(path.resolve(BBL_BA_SRC, "../Untitled design (37).png")).resize({
    width: 1000,
