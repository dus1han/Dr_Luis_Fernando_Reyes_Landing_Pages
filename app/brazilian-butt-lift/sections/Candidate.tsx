      ...
      <div className="shell">
        <div className="grid gap-[38px] lg:grid-cols-[0.92fr_1.08fr] lg:gap-[70px]">
          <div>
            ...
            />
            {/* Two paragraphs here, one on the buccal page — the brief for
                this procedure splits the answer, and running the halves
                together buries the second. `mb-0` on the last so the
                column still ends level with the checklist beside it. */}
            {CANDIDATE.intro.map((p, i) => (
              <Reveal
                key={i}
                as="p"
                delay={0.08 * i}
                className="max-w-[50ch] text-body last:mb-0"
              >
                {p}
              </Reveal>
            ))}
