...
> **Delivery is email**, in `lib/lead-mail.ts`. A new field reaches the clinic
> with no change there either — the text and HTML bodies are built from the
> lead object, but only fields named explicitly are rendered, so add it to
> `buildText`/`buildHtml` if it should appear. The subject carries the page
> name from `lib/pages.ts`: `[Buccal Fat Removal] New consultation request —
> <name>`. Credentials are runtime `SMTP_*` keys in `.env` on the VPS; see
> `docs/deployment.md`.
...
| `/api/lead` | `app/api/lead/route.ts` | Form handler. Dynamic (`runtime = "nodejs"`), emails the clinic via `lib/lead-mail.ts` |
