...
/*
 * Two variants of the same lockup, not one plus a CSS filter.
 *
 * The nav sits on ivory and the mobile menu panel behind it is espresso,
 * so the logo has to work on both. It used to be the ink recolour with
 * `brightness-0 invert` applied while the panel was open — fine for a flat
 * silhouette, wrong for this artwork: inverting the gold monogram turns it
 * blue. Swapping the source keeps the gold gold.
 */
const logoDark = SHARED["logo-full.png"];
const logoLight = SHARED["logo-light.png"];
    ...
    <>
      ...
      >
        <div className="shell flex h-(--nav-h) items-center justify-between gap-6">
          ...
          >
            <Image
              /* Swapped, not filtered — see the note on the two constants
                 at the top of this file. The panel behind the open menu is
                 espresso, where the near-black half of the lockup would
                 disappear. */
              src={open ? logoLight.src : logoDark.src}
